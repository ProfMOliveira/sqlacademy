...
type Lesson = { id:string; module:string; level:Level; title:string; desc:string; sql:string; tip:string; xp:number; preview?:string };
...
const lessons: Lesson[] = [
 {id:'create-table',module:'Fundamentos',level:'Básico',title:'CREATE TABLE',desc:'Crie tabelas e defina colunas.',sql:`CREATE TABLE cursos (\n  id INTEGER PRIMARY KEY,\n  nome TEXT NOT NULL,\n  carga_horaria INTEGER CHECK (carga_horaria > 0)\n);`,tip:'PRIMARY KEY identifica o registro. NOT NULL exige um valor. CHECK valida uma regra.',xp:40,preview:`SELECT name, type FROM sqlite_master WHERE type='table' AND name='cursos';`},
 {id:'insert',module:'Fundamentos',level:'Básico',title:'INSERT INTO',desc:'Insira uma ou várias linhas.',sql:`INSERT INTO alunos (id, nome, idade, curso)\nVALUES (6, 'Fabio', 23, 'SQL');`,tip:'Informe as colunas explicitamente para tornar o INSERT mais seguro e legível.',xp:40,preview:`SELECT id, nome, idade, curso FROM alunos WHERE id=6;`},
...
export default function SqlTrainer(){
 ...
 useEffect(()=>{(async()=>{try{const SQL=await initSqlJs();const db=new SQL.Database();db.run(seed);dbRef.current=db;setReady(true);setCols([]);setRows([]);}catch(e){setError(true);setMessage(String(e));}})();},[]);
 ...
 async function runLesson(l:Lesson){
  try{
   ...
   db.run(seed);
   dbRef.current=db;
   const result=db.exec(l.sql);
   const display=result.length ? result : (l.preview ? db.exec(l.preview) : []);
   if(display.length){
    setCols(display[0].columns);
    setRows(display[0].values.map((v:unknown[])=>Object.fromEntries(display[0].columns.map((c:string,i:number)=>[c,v[i]]))));
    setMessage(`${display[0].values.length} linha(s) exibida(s).`);
   }else{setCols([]);setRows([]);setMessage('Comando executado com sucesso.');}
   setError(false);
  }catch(e){setError(true);setMessage(e instanceof Error?e.message:String(e));setRows([]);setCols([]);}
 }
 function reset(){if(!dbRef.current)return;dbRef.current.close();const SQLPromise=initSqlJs();SQLPromise.then(SQL=>{const db=new SQL.Database();db.run(seed);dbRef.current=db;setRows([]);setCols([]);setError(false);setMessage('Banco restaurado.');setCols([]);setRows([]);});}
 function selectLesson(l:Lesson){setTab('aulas');setActive(l.id);setCode(l.sql);setMessage('Exemplo carregado. Clique em Executar para rodar o comando.');setError(false);setCols([]);setRows([]);setShowLessonHint(false);setMobile(false);}
 ...
 return <div className="grid xl:grid-cols-[.85fr_1.15fr] gap-4">
  ...
  {showHint && <div className="fixed inset-0 z-50 bg-black/65 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
   <div className="w-full max-w-lg rounded-2xl border border-amber-300/30 bg-[#101827] shadow-2xl overflow-hidden">
    ...
    <div className="p-5 space-y-5"><div><div className="text-xs uppercase tracking-wider text-amber-300 font-semibold">Como pensar</div><p className="text-sm text-slate-300 mt-2 leading-6">{hint}</p></div><div><div className="text-xs uppercase tracking-wider text-indigo-300 font-semibold">Como o comando funciona</div><p className="text-sm text-slate-300 mt-2 leading-6">{exercise.explanation}</p></div><div><div className="text-xs uppercase tracking-wider text-emerald-300 font-semibold">Resposta</div><pre className="mt-2 rounded-xl bg-[#080d17] border border-[#243149] p-4 text-xs text-emerald-200 overflow-auto whitespace-pre-wrap">{exercise.expected}</pre></div></div>
