...
export default function SqlTrainer(){
 ...
 useEffect(()=>{(async()=>{try{const SQL=await initSqlJs();const db=new SQL.Database();db.run(seed);dbRef.current=db;setReady(true);setTimeout(()=>runLesson(lessons[0]),0);}catch(e){setError(true);setMessage(String(e));}})();},[]);
 ...
 function reset(){if(!dbRef.current)return;dbRef.current.close();const SQLPromise=initSqlJs();SQLPromise.then(SQL=>{const db=new SQL.Database();db.run(seed);dbRef.current=db;setRows([]);setCols([]);setError(false);setMessage('Banco restaurado.');setTimeout(()=>runLesson(lessons[0]),0);});}
 function selectLesson(l:Lesson){setTab('aulas');setActive(l.id);setCode(l.sql);setMessage('Exemplo carregado.');setError(false);setShowLessonHint(false);setMobile(false);void runLesson(l);}
